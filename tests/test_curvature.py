...
class TestExpressionCurvature:
    ...
    def test_pow_curvature(self):
        ...
        # AFFINE^2 = CONVEX (even powers of affine are convex regardless of sign)
        expr = x**2
        assert expr.curvature == C.CONVEX

        # AFFINE^4 = CONVEX (even powers)
        expr = x**4
        assert expr.curvature == C.CONVEX

        # CONVEX^2 = UNKNOWN (requires positivity analysis to verify non-negativity)
        convex_expr = norm(x)
        expr = convex_expr**2
        assert expr.curvature == C.UNKNOWN

        # CONCAVE^0.5 = UNKNOWN (requires positivity analysis)
        concave_expr = -norm(x)
...
class TestAtomCurvature:
    ...
    def test_function_curvature(self):
        ...
        # FunctionExpr always returns UNKNOWN
        x = Variable(shape=(3,))
        x.value = np.array([1.0, 2.0, 3.0])
        func = Function(test_func)
        expr = func(x)  # Call to get FunctionExpr
        assert expr.curvature == C.UNKNOWN
...
class TestComplexCurvatureCombinations:
    ...
    def test_curvature_chain_rules(self):
        ...
        # Chain: affine -> pow(2) -> mul by negative -> add affine
        # Note: CONVEX^2 is NOT always convex (counterexample: (x²-100)²)
        # But AFFINE^2 is always convex
        expr1 = x[0]  # AFFINE
        expr2 = expr1**2  # CONVEX (even power of affine)
        expr3 = -expr2  # CONCAVE
        expr4 = expr3 + y[0]  # CONCAVE + AFFINE = CONCAVE
        assert expr4.curvature == C.CONCAVE
...
class TestCurvatureEdgeCases:
    ...
    def test_fractional_powers(self):
        ...
        # AFFINE^(0.5) = UNKNOWN (requires positivity analysis)
        expr = x**0.5
        assert expr.curvature == C.UNKNOWN

        # AFFINE^(1.5) = UNKNOWN (requires positivity analysis)
        expr = x**1.5
        assert expr.curvature == C.UNKNOWN
